...
Se usa `vector(1536)` para ser compatible con `text-embedding-3-small` de OpenAI, modelo multilingüe de alta calidad para texto jurídico en español. El índice HNSW con similitud coseno es el estándar para búsqueda semántica en pgvector.
...
| embedding    | vector(1536)  | nullable                             | Vector semántico generado con text-embedding-3-small (OpenAI) |
