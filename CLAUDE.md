...
1. **[✅ CORREGIDO 12/07/2026] Sesgo de la opción "a".** El **86,4%** de las 88 preguntas IA tenían la correcta en la **"a"** (b: 6,8% · c: 5,7% · d: 1,1%; el examen real: 27,8%). **Corrompía todo**: el alumno aprendía "ante la duda, marca la a", acertaba sin saber la ley, y eso **invalidaba nuestras métricas** (dominio, dificultad empírica, "¿estoy listo?"), que medirían a alguien adivinando el patrón.
   - [✅ `scripts/equilibrar_clave.py`] Barajado **dirigido** (no aleatorio: el aleatorio deja el reparto desigual por azar) para que la clave quede al **25,0% exacto** en cada letra. Las 88 corregidas. **Las oficiales NO se tocan**: son el examen real.
   - [⚠️ Detalle no obvio] **59 de las 88 explicaciones citaban las opciones por letra** ("La opción b) es incorrecta porque…"). Barajar sin más las habría dejado **señalando a la opción equivocada**. El script **remapea las letras dentro del texto**, con un patrón que **no toca las referencias a artículos** (`artículo 10.2.c)`: la letra no puede ir precedida de dígito ni punto). Validación automática: aborta si alguna explicación acabara llamando "incorrecta" a la opción correcta. Resultado: **0 incoherencias**.
   - [✅ Causa raíz] `build_test_bank._parse_and_validate()` ahora **baraja al generar** (`_barajar_opciones`), así que no puede repetirse. Verificado con 400 generaciones simuladas: 23,8 / 24,0 / 25,2 / 27,0 %.
2. **[✅ HECHO 12/07/2026 — migración 048] Guardar QUÉ opción eligió el alumno.** Antes `progreso_usuario` solo guardaba acierto/fallo: **se tiraba la señal pedagógica más valiosa** (qué distractor le engañó). Y los datos que no se guardan **no se recuperan mañana**.
   - `normas.respuestas`: **una fila por cada respuesta** (no por pregunta, como `progreso_usuario`, que guarda el ESTADO del SM-2 — esto guarda el HISTORIAL). Campos: `opcion_elegida` (**NULL = en blanco**), `correcta`, `contexto` (repaso / prueba_nivel / simulacro_personal / simulacro_academia), `segundos` (para el futuro cronómetro).
   - **Se registra también la respuesta EN BLANCO**, y no es un detalle: con la fórmula **A−E/3** dejar en blanco es una **decisión estratégica**, no una no-respuesta. Sin ese dato no se puede entrenar la destreza de "cuándo callar" (punto 4). CHECK en BD: una respuesta en blanco nunca puede ser correcta.
   - [✅ `retrieval.registrar_respuesta()`] Enganchado en **los 4 modos** del alumno.
   - [✅ `retrieval.analisis_distractores()`] Ya operativo: detecta **distractores muertos** (nadie los elige → la pregunta es de 3 opciones en la práctica, más fácil de lo que parece) y preguntas **sospechosas** (un distractor gana a la correcta → o la clave está mal marcada, o la pregunta es ambigua). Probado con 12 respuestas simuladas.
   - [✅ Verificado en la app] Responder en Repaso crea filas en `normas.respuestas`, **incluidas las dejadas en blanco**.
3. **[✅ HECHO 12/07/2026 — migración 049] Discriminación del ítem.** La dificultad es solo **la mitad** de la teoría clásica de test. La otra mitad: ¿la pregunta **distingue** al que sabe del que no? Discriminación **negativa** (los buenos la fallan MÁS que los malos) = pregunta **rota**: ambigua, o **con la clave mal marcada** — el error más grave y el más difícil de ver a ojo. Es un **detector automático de preguntas defectuosas**; ningún editor puede cazar eso revisando 1.800 preguntas a mano. Control de calidad del banco = **argumento B2B**.
   - [✅ `scripts/analisis_items.py`] Correlación **biserial-puntual** entre acertar el ítem y la capacidad general del alumno, con la capacidad **corregida** (excluyendo el propio ítem: si no, la pregunta se correlaciona consigo misma y el resultado sale inflado). Tres alertas: `clave_sospechosa` (grave) · `no_discrimina` · `distractor_muerto`.
   - [✅ **Validado plantando un defecto**] Se generaron 30 alumnos sintéticos y se **rompió a propósito una pregunta** (clave mal marcada). El detector la cazó: **discriminación −0,44**, señalada como `clave_sospechosa`, y **ninguna falsa alarma grave** entre las otras 11. Datos sintéticos borrados después.
   - [⚠️ Umbral corregido durante la validación] La primera versión marcaba como grave cualquier `d < 0`, y una pregunta sana con **−0,02** (puro ruido muestral) saltaba como "clave sospechosa". **Un detector que grita 'lobo' no sirve**: el editor acabaría ignorando las alertas. Ahora la alerta grave exige `d < −0,10`.
   - [✅ `streamlit_app.py`] Sección **"Calidad del banco"** en el modo "Banco y pesos" (solo admins), con las preguntas marcadas.
   - ⚠️ **NECESITA ESCALA.** Con 2 alumnos no dice nada: no hay variabilidad de capacidad que correlacionar. Madura con decenas de alumnos (una academia). Umbral en BD (`min_respuestas_discriminacion`).
4. **[✅ HECHO 12/07/2026 — migración 050] Calibración de confianza / cuándo dejar en blanco.** Medíamos si el alumno acierta. **No medíamos si SABE lo que sabe** — y con la fórmula **A−E/3** eso es lo que decide puntos.
   - [✅ Migración 050] `respuestas.confianza` (`seguro`/`dudo`/`ni_idea`), preguntada **antes** de corregir. Sin ese dato, **acertar por saberlo y acertar por suerte son idénticos en la BD**, así que ninguna de las dos patologías de abajo se puede detectar.
   - [✅ `retrieval.get_umbral_rentabilidad()`] El umbral **sale de `convocatorias`, no está hardcodeado**: `p* = (pen_error − pen_blanco) / (valor_acierto + pen_error)`. Con GACE = **25,0%**, que es *exactamente* el azar de 4 opciones. Es decir: **responder al azar es NEUTRO**, y descartar **una sola** opción (p = 1/3) ya hace que responder **gane** puntos. **"Ante la duda, en blanco" es matemáticamente falso** con esta fórmula — y eso se le enseña al alumno con su propio dato.
   - [✅ `retrieval.get_calibracion()`] Caza las dos patologías caras: **exceso de confianza** (falla diciendo "lo sé" → *nunca lo repasará*, porque cree dominarlo: el fallo más caro que existe) y **exceso de prudencia** (deja en blanco lo que habría acertado → regala puntos). Más la calibración **monótona** y los **puntos/pregunta** por nivel.
   - [✅ `streamlit_app.py`] Selector 🟢/🟡/🔴 en **Repaso** y **Simulacro personal**; aviso en el sitio al fallar estando seguro; panel **"¿Sabes lo que sabes?"** en Mi progreso. **No** en la prueba de nivel (40 preguntas ya son bastante fricción en el onboarding) ni en el simulacro de academia (es su examen, no un entrenamiento).
   - [⚠️] Sin `min_respuestas_calibracion` (10, en BD) respuestas en un nivel **no se diagnostica nada**: un consejo mal fundado es peor que ninguno.
   - [✅ **Validado plantando las dos patologías**] Alumno de laboratorio con 58,3% de acierto diciendo "seguro" y 41,7% con "ni idea" + 6 blancos: **las dos cazadas**. Verificado además en navegador real (umbral 25,0% leído de BD, aviso de "dijiste que lo sabías", panel completo). Datos borrados.
5. **[✅ HECHO 12/07/2026 — migración 051] Práctica intercalada.** El repaso era por tema, **en bloque** — y la práctica en bloque produce **fluidez falsa**: mientras machacas LPAC tienes su esquema cargado, aciertas, y concluyes que te la sabes. El examen te la mezcla con la LRJSP y ahí se ve que no.
   - [✅ Migración 051] `normas.pares_confundibles`. **Los pares no se escriben a mano: se derivan de `epigrafe_leyes`** (dos leyes son confundibles si el programa las estudia juntas), con **Jaccard** y no conteo bruto — el conteo premia a las leyes **ubicuas** (CE1978 está en 28 temas: comparte con todo y no se confunde con todo).
   - [✅ `scripts/calcular_confundibles.py`] 19 pares en GACE, todos reconocibles: **LPAC/LRJSP (0,75)**, LGS/RLGS (0,80), TUE/TFUE (1,00), TREBEP/RIRS, LGP/LOEPSF. **La fórmula sola encontró el par que el usuario citó de memoria.**
   - [✅ `retrieval.get_grupos_intercalados()` + `_intercalar()`] Solo se ofrece cuando el alumno **ya tiene base en las DOS leyes** (`min_vistas_intercalada`=5, en BD). Y **no se baraja al azar**: el azar deja rachas, y una racha de cuatro LPAC seguidas *es* la práctica en bloque que se quiere evitar. Alterna estrictamente.
   - [✅ `streamlit_app.py`] Selector **Por tema / Intercalada** en Repaso, con **desglose de acierto por ley** al terminar: si una de las dos se hunde, ahí está la confusión.
   - [⚠️ **Dificultad deseable**] Intercalar **empeora el rendimiento mientras se practica** y lo **mejora en el examen**. Por eso (1) **no se le sirve a un principiante** en esa materia (mezclar sin base impide construir la primera representación) y (2) **se le avisa de que va a fallar más**, o creerá que la app ha empeorado y volverá al modo que le engaña.
   - [✅ Verificado en navegador real] Tanda LPAC-LRJSP-LPAC-LRJSP… con **0 rachas**, aviso mostrado, desglose por ley correcto. Sin errores de consola.
6. **[✅ HECHO 12/07/2026 — migración 052] Cronómetro.** El simulacro no cronometraba: simulaba el examen **menos la restricción que más gente tumba**. Y sin reloj no se puede entrenar la destreza de **repartir el tiempo** — atascarse en la 12 y no llegar a las 20 últimas cuesta más puntos que no saberse un tema.
   - [✅ `retrieval.get_tiempo_simulacro()`] La duración **no se hardcodea**: sale de `convocatorias` (`tiempo_minutos`/`num_preguntas` = **54 s/pregunta**), así que 50 preguntas heredan 45 minutos y, si cambia la convocatoria, el simulacro cambia solo.
   - [✅ Entrega automática al llegar a cero] Como el examen real: no hay prórroga. Cuenta atrás con `st.fragment(run_every=1)` — se repinta **solo el reloj**, no las 50 preguntas. El tiempo corre contra la marca del **servidor**, no contra un contador: sigue corriendo aunque el alumno cierre la pestaña.
   - [⚠️ **El blanco que no es una decisión**] Con reloj aparece un blanco nuevo: el de la pregunta **a la que no llegó**. En BD se veía idéntico al blanco estratégico (migración 050), y confundirlos rompía tres cosas: (1) `get_calibracion()` le diría *"eres demasiado prudente, contesta más"* al que en realidad **va lento** — el consejo al revés; (2) **envenenaba la dificultad empírica y la discriminación del ítem**, porque las preguntas del **final** son a las que menos gente llega y **parecerían imposibles por su posición**, no por su contenido; (3) falseaba el análisis de distractores. De ahí `respuestas.blanco_por_tiempo`, ya excluido en `get_calibracion`, `analisis_distractores` y `scripts/analisis_items.py`. Criterio **conservador**: solo se marca la que **no tocó en absoluto**; si la vio y calló, eso es una decisión y se respeta.
   - [⚠️ **Fallo de método cazado en la verificación**] `ritmo = segundos / n_preguntas` hacía parecer **puntual al que va lento**: quien agota el reloj consume el tiempo entero, así que le salía clavado el objetivo (54,0) y la app le decía *"vas justo, pero llegas"* **bajo el aviso de que se había quedado sin tiempo**. El denominador honrado son las preguntas **a las que llegó** (`historial_simulacros.sin_llegar`): el mismo intento pasa de "54 s/pregunta, vas bien" a **"108 s/pregunta, margen −100%"**.
   - [✅ `respuestas.segundos`] Vacío desde la migración 048, ya se rellena (se sella al contestar por primera vez). **Es una aproximación y así está documentado**: en una página con 50 preguntas donde se puede saltar y volver, el rato de leer las que salta se le atribuye a la siguiente que contesta.
   - [✅ Verificado en navegador real] Personal: reloj de 45:00 descontando, tiempos sellados, intento guardado. Academia de 2 preguntas (108 s) **dejando que el reloj llegara a cero**: entrega automática a los 108 s exactos, `tiempo_agotado=TRUE` y la pregunta no tocada marcada `blanco_por_tiempo=TRUE`. Sin errores de consola.
...
4. **Ritmo por bloque y por tema** — *"¿en qué materia se te va el tiempo?"*. **Los segundos ya se guardan** (migración 052); lo que falta es el agregado, y **hoy no hay ni una sola respuesta cronometrada real** que agregar. **Disparador: 4-6 simulacros cronometrados por alumno.** Antes de eso sería una gráfica bonita construida sobre 3 respuestas — el mismo error que la dificultad heurística, y esta vez el dato **llega solo** con el uso. Lo valioso es el **cruce ritmo × acierto**: *lento y acierta* es el caso que más puntos regala (se lo sabe, pero no llega al final). Detalle y checklist en `TODO.md` § 3.bis.
