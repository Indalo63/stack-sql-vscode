...
### Completado — PostgreSQL local vía Docker Desktop (18/07/2026)

Planteado el 16/07/2026 como observación futura; ejecutado el 18/07/2026. Contexto real encontrado al empezar: ya existía un `docker/docker-compose.yml` (Postgres 16 + pgvector + pgAdmin) de un experimento anterior del usuario, no documentado aquí — se reutilizó en vez de partir de cero, tras confirmar con él qué era.

- [✅] **Volumen de datos reseteado.** El volumen (`docker/data/postgres_pgvector`) tenía un esquema genérico de una plantilla muy anterior (`sales.customers`, `examenes.*`, `legislacion.*` — de los ficheros `001`/`002`/`010` de `sql/ddl/`, nunca purgados del repo). Se conservó el `docker-compose.yml` (imagen, puertos, pgvector) y se borró solo el volumen.
- [✅] **Las 37 migraciones (`sql/ddl/001`–`052`) se aplicaron en orden** contra `stack_db` local. `CREATE EXTENSION vector` hubo que crearla a mano antes de la 002: en Supabase se activó en su día desde el toggle del dashboard, no desde ningún fichero DDL, así que ese paso no está en el historial de ningún migration file.
- [⚠️ **Hallazgo — dependencia oculta entre migraciones y datos reales**] La migración `020_banco_preguntas.sql` inserta en `oposicion_leyes` asumiendo que `normas.leyes` ya tiene filas (esos datos llegan por `load_ley.py`, no por DDL). En una BD nueva sin leyes, **falla con violación de FK** — hubo que sembrar primero las 6 leyes núcleo (CE, LPAC, LRJSP, TREBEP, LGP, LCSP) desde Supabase (solo lectura) antes de continuar.
- [⚠️ **Hallazgo más sutil — fallo silencioso, no ruidoso**] Las migraciones `025`, `026`, `042`, `044` (repertorio completo de 60 leyes y pesos) usan `EXISTS(...)`/`ON CONFLICT DO NOTHING`, así que **no fallan** si faltan leyes — simplemente insertan menos filas de las que tocan, sin avisar. Con solo 6 leyes sembradas, `oposicion_leyes` quedó en 6/60 tras aplicar todo el bloque 001–052 **sin ningún error en pantalla**; solo se detectó comparando conteos contra lo documentado aquí. **No se tocan esas migraciones** (son historial ya aplicado en Supabase, y ahí siempre tuvieron datos completos delante): el riesgo solo existe al reconstruir desde cero, y ya queda anotado aquí para la próxima vez.
- [✅] **Datos reales completados en local** (solo lectura desde Supabase, vía `scripts/_supabase_env.py::load_supabase_secrets()` + Session Pooler): `leyes` (60), `oposicion_leyes` (60, pesos sumando 100), `titulos` (379, dependencia de FK de `preguntas_test.titulo_id`), `preguntas_test` (297), `epigrafe_leyes` (357). Re-ejecutar `025`/`026`/`042`/`044` después de completar `leyes` bastó para rellenar `oposicion_leyes` (son idempotentes).
- [✅ Migración `053_leyes_columnas_sync_boe.sql`, nueva] `normas.leyes.content_hash` y `.fecha_actualizacion` **existían en Supabase sin ninguna migración que las creara** — alguien las añadió a mano desde el SQL Editor en su momento. Creada la migración que faltaba (`IF NOT EXISTS`) para cerrar el hueco del historial. Aplicada en local y en Supabase (confirmado no-op allí: mismas dos columnas, sin cambios reales).
- [✅] Rol `app_asistente` (migración 040) recreado en local sin cambios — no depende de datos, solo de las migraciones.
- **Lo que mejoró, confirmado:** sin Session Pooler en local (conexión directa desde el devcontainer vía `host.docker.internal`, o `localhost` desde una terminal WSL nativa — ver `scripts/_supabase_env.py::use_local_defaults()`, que ya existía de antes por este mismo motivo).
- [✅ Paso 4] **Alternar la app entre Supabase cloud y el Postgres local sin arriesgar apuntar a producción por error**, resuelto con `scripts/db_switch.py`: `secrets.toml` pasa a ser un symlink a `secrets.supabase.toml` o `secrets.local.toml` (gemelos salvo en el bloque `DB_*`), así que el estado es inspeccionable (`db_switch.py status`) y no hay edición manual que pueda dejar las credenciales a medio pisar. `app/config.py` expone `DB_ENTORNO` (derivado del host, no de una variable aparte) y el sidebar pinta un aviso en rojo/verde ("🔴 BD: SUPABASE (producción)" / "🟢 BD: Postgres local") como primera línea de defensa visual.
- **Fuera de alcance, a propósito:** Supabase Auth (login de alumno) no se replicó — seguiría dependiendo de Supabase cloud aunque la BD sea local. Los embeddings de artículos/fragmentos del corpus de Q&A tampoco se copiaron (no se pidió; sería el siguiente nivel de réplica si hiciera falta probar Q&A en local).
